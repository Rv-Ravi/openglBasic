...
struct LightEntity {
	...
	float m_dist, m_inAngle, m_outAngle;
};
...
void main() {
	...
	float litAngle = dot(normalize(light.m_direction),-lightDir);

	if (litAngle > light.m_outAngle)
	{
		//float atenFac = 1.f / (1.f + (0.007f * dist) + (dist * dist * 0.0032f));
		float atenFac = clamp(light.m_dist / dist, 0.f, 1.f);
		float angleFac = clamp((litAngle - light.m_outAngle) / (light.m_inAngle - light.m_outAngle), 0.f, 1.f);
		...
		frgColor = (vec4(light.m_ambientIntensity, 1.f) + diffFac + specFac) * atenFac * angleFac;
	}
	else {
		frgColor = vec4(light.m_ambientIntensity, 1.f) * texture(material.m_diffuse, tCord);
	}
